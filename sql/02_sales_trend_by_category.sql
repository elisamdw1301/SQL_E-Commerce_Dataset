...
SELECT    
    p.category, 
-- Calculating the amount of the values in the `quantity` column based on the year in the `order_date` column
...
-- Combining the `order_detail` and `product_detail` tables based on the same `sku_id` column
JOIN 
